...
class Qwen(BaseChatModel): 
    ...
    def __init__(self): 
        self.model_adapter: QwenModelAdapter = self._get_model_adapter()
        self.model, self.tokenizer = self._get_model_tokenizer()
        self.prompt_adapter: QwenPromptAdapter = self._get_prompt_adapter()
    ...
    def _get_model_adapter(self): 
        """获取模型适配"""
        qwen_model_adapter = QwenModelAdapter()
        return qwen_model_adapter
    
    def _get_prompt_adapter(self): 
        """获取提示词适配"""
        qwen_prompt_adapter = QwenPromptAdapter()
        return qwen_prompt_adapter
        ...
        for i in range(max_new_tokens): 
            ...
            if i % stream_interval == 0 or i == max_new_tokens - 1 or stopped:
                ...
                output = tokenizer.decode(
                    tmp_output_ids,
                    skip_special_tokens=False,    # Qwen 这里是 False
